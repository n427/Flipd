...
async function usersAreBlocked(userId: string, otherUserId: string): Promise<boolean> {
...
export async function GET(
  ...
  if (!owner && (effectiveWantedStatus(data.status, data.needed_by, now) !== 'active'
    || await usersAreBlocked(user.id, data.buyer_id))) {
...
export async function DELETE(
  ...
  if (existingError || !existing) return NextResponse.json({ error: 'not found' }, { status: 404 });
  if (existing.buyer_id !== user.id) return NextResponse.json({ error: 'forbidden' }, { status: 403 });
  if (existing.status === 'deleted') return NextResponse.json({ error: 'not found' }, { status: 404 });

  const resolvedAt = new Date().toISOString();
  const { error: postError } = await supabase
    .from('wanted_posts')
    .update({ status: 'deleted', resolved_at: resolvedAt })
    .eq('id', id)
    .eq('buyer_id', user.id)
    .neq('status', 'deleted');
  if (postError) return NextResponse.json({ error: postError.message }, { status: 500 });

  // Accepted offers retain their message_threads. Only open competitors close.
