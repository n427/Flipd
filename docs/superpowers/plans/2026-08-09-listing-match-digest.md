...
Structure it exactly as below — the ordering of the gates and the stamp is the whole correctness story:
...
export const digestProducer: Producer = {
  ...
  async run() {
    const now = new Date();
    // Cheapest gate first: outside the window there is nothing to do at all,
    // and no row should be read or stamped.
    if (!isInSendWindow(now)) return { digests: 0, skipped_window: 1 };

    // ... fetch candidate listings once (last 24h, shared across all users)
    // ... fetch users whose notify_prefs allow listing_match
    // For each user, in sequence:
    //   1. skip unless isDue(user.last_digest_at, now)
    //   2. gather signals; buildProfile; skip if null (cold start)
    //   3. matchListings; skip if empty (no matches = no notification)
    //   4. send the email
    //   5. ONLY THEN stamp last_digest_at
    // Wrap each user in try/catch: a thrown error skips that user and must
    // NOT stamp them, so tomorrow's run retries rather than silently
    // dropping them forever.
    return { digests: sent, errors: failed };
  },
};
```
