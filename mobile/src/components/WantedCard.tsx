...
export function WantedCard({ post, onPress }: { post: WantedPost; onPress: () => void }) {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={`${post.title}, ${copy.budget}, posted ${new Date(post.created_at).toLocaleDateString()}`} onPress={onPress} style={{ backgroundColor: '#fff', borderWidth: 1, borderColor: T.rule, borderRadius: 18, overflow: 'hidden', marginBottom: 14 }}>
    ...
    <View style={{ padding: 16, gap: 7 }}>
      ...
      <Text style={{ fontFamily: F.medium, color: T.muted, fontSize: 12.5 }}>Posted {new Date(post.created_at).toLocaleDateString()}</Text>
